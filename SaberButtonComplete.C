...
// Base Program
	...
	while(1) {
		...
		if(LIGHT){
			...
			if(A2D < 255){RED=255; GREEN=0; BLUE=0; PORTC=6;}
			else if((A2D > 256)&&(A2D < 511)){RED=0; GREEN=255; BLUE=0; PORTC=5;}
			else if((A2D > 512)&&(A2D < 767)){RED=0; GREEN=0; BLUE=255; PORTC=3;}
			else if(A2D > 768){RED=255; GREEN=255; BLUE=255; PORTC=0;}
			show = 1;
			N = 0;
